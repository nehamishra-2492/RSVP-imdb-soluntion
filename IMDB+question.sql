...
-- Action has the highest avg_duration of 112.88 followed by Romance, Crime, Drama
-- Horror has the least avg_duration of 92.72
...
-- movie with median_tating 7 has the highest movie_count of 2257
...
-- there rae 8  movies starting with 'The'
...
WITH top_3_genre AS
( 	
	SELECT `g`.`genre`, COUNT(`movie_id`) AS number_of_movies
    FROM `genre` AS g
    INNER JOIN `movie` AS m
    ON `g`.`movie_id` = `m`.`id`
    GROUP BY `g`.`genre`
...
SELECT *,
	RANK()
    OVER(ORDER BY 'movie_rank' DESC) AS prod_comp_rank
